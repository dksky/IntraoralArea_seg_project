...
def testGeneratorForGreyImg(img,target_size = (256,256),flag_multi_class = False,as_gray = True):
    img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    img = img / 255
    img = cv2.resize(img, target_size)
    #img = trans.resize(img,target_size)
...
def testGeneratorForRBGImg(img,target_size = (256,256),flag_multi_class = False,as_gray = True):
    img = img / 255
    img = cv2.resize(img, target_size)
    #img = trans.resize(img,target_size)
    img = np.reshape(img,(1,)+img.shape)
